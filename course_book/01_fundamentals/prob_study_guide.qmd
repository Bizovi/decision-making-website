...
If we have multiple independent events, each one being a coin flip, we can model the number of successes $k$ out of $n$ trials with the Binomial distribution. In order to apply what we learned about combinatorics, a good exercise is to derive its distribution (probability mass function) via a story proof.

::: {.callout-note}
## Couples showing up to a safari

We will analyze and simulate a simple, but tricky example to find out what is the probability of a given number of people who signed up for a safari to actually show up. I take as a starting point A. Fleischhacker's [example](https://www.persuasivepython.com/1-repuncertainty) from his book called "Persuasive Python".
:::

In our version of the problem, we also have 3 seats available, but this time the events are not independent, as we have a proportion of people making reservations for two (couples or friends). We assume that if one of them changes their mind and doesn't show up, neither will the other. Moreover, even if we know the prevalence of couples in the population of our customers, we still have to calculate the proportion of seats reserved by each type. 

In order to overcome these challenges, we will use probability trees, Bernoulli and Binomial distributions. We'll end up with a nasty formula which can be interpreted as a mixture model. Then, we simulate the process and interpret the results, comparing them to the case in which all passengers were independent. 

To take our analysis even further, we can do a sensitivity analysis which will tell us for what proportion of couples in the population we start seeing dramatically different outcomes in terms of expected number of people who show up. There are multiple ways in which we can compute it, but I will leverage the formulas for calculating the expected value and variance for an arbitrary discrete random variable. 

This will enable us to compute the confidence intervals around the mean, which tells us if we're to repeat the simulation many times, 95% of the confidence intervals will contain the true mean. In the safari problem, computing confidence intervals doesn't make too much sense, but we will explore this important idea in much more depth in later lectures. As a rule of thumb, always ask yourself "What if I got really lucky or unlucky with this sample?"

This case-study teaches you a lot of important concepts and techniques in probability. I also introduce the Law of Large Numbers to justify why simulation works and what are the dangers when we don't think about sample size. You also need to be aware about a little theorem, called "Universality of the Uniform", which states that using good pseudo-random numbers, uniformly distributed samples, we can generate samples from any distribution. 

In terms of programming, you will learn to work with data frames, functions, use vectorized calculations instead of for loops, manipulate data, make persuasive visualizations, and generate reproducible reports of your work.
...
This is a true rabbit hole. Right now, we have the tools to simulte what can we expect if there was no effect or a small effect, but we can't design yet a statistical test. For that, refer to the last section in Tableau 10 of S. Venkatesh, Kahneman's original paper, and recent papers which challenge the statistical methodology and modeling approach based on which the original conclusion was reached.
:::

Despite the ongoing debate about the magnitude of the effect in the hot hand phenomenon, it is undeniable that such simulations are extremely valuable -- they prevent us to be fooled by randomness. To take a few examples:

- we might see patterns in stock markets which can be explained by a random walk
- we might not realize the effect of the sample size on observing extreme results
- we might attribute an observed pattern of ELO and performances by gender in chess to nonsensical explanations, when it can be explained by a disparity in the number of players, combined with estimating a max / top k proportion
- we might underestimate the support for a political party due to differential non-response
- we might overestimate the effectiveness of a medical treatment due to drop-out and censoring
