...
flowchart TD
    ...
    U -- hypothesis testing --> NP[7. Neyman-Pearson] --> Persp[8. Freq, Lik, Bayes] --> RC[9. Replication Crisis]
...
## Combinatorics. Sampling. Urn Models
 ...
 replacement, multiplication rule, naive $\mathbb{P}$ | Birthday Paradox, Bootstrap |
...
Neyman-Pearson frequentism. Popper and falsification | Type I, II errors, confint, p-value, PPV, effect sizes, **power analysis, range predictions**, $H_0$, $H_A$ | How to ask better questions. Stahel's relevance  |
...
Refactor and edit
- A/B Testing pitfalls | A/A Tests, Selection bias, Confounders, Novelty | The need for rigorous experiments |
- Error Types, Effect Size, p-values | tests for difference in proportions and means | Strategy for simulation |
Power, Sample Size calculations, Non-inferiority tests | relevance, minimal effect size | Neyman-Pearson, frequentism as action in the long-term |
Linear regression review | Hypothesis tests as linear models | Guidance with student projects |
Properties of Metrics for A/B Testing | Sensitivity, stability, directedness | Potential pitfalls |
CLT, Choosing a test, Trading off errors, Increasing Power | Cohen's d, PPV, GLM, nonparametrics | Justifying $\alpha, 1 - \beta, \Delta, n$ by simulation |

Error types, p-values, error control
GQ(Good Questions) --> Pre[Pre-Registration]
Publication bias, open science
Significant findings, sample size justification and power analysis
Effect sizes, cohen's d, minimal effect of interest, telescope method, resource-based
...
- Popper and philosophy of science, falsification, theory construction
