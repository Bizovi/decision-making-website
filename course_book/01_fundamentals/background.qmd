...
Where do you find it all in the wild and practice?
Logistics and supply chain: how much inventory should I order from manufacturing, to satisfy the demand (that last item on the shelf) and to avoid excess inventory.
...
Now, if we talk about AI (what we understand by it changed historically), to the definition, we would add the autonomy (weak ai), not self-awareness (general AI). We already gave two examples -- one in e-commerce, and second as pandemic-response. We have to keep in mind that what we do is very domain-dependent/specific (we call it domain-driven). Can you give some examples of businesses, problems, sevices which you suspect do have algorithms/models of AI/Ml behind the scenes:

A: 
- liverpool AI/ML won title strategy and tactics discovery on the field -- sports' science. NBA (invested a lot in data infrastructure, ML, e.g. lakers/rockets/golden state), baseball. Lakers -- what is the best player for that position? (Thinking basketball). Look, again, they had to make a decision!
- dynamic pricing in bolt/uber, which takes into account rain/weather, peak hours: balancing demand and supply. It is at the intersection of ML/Econ theory, as they are a marketplace. Also, when it doesn't work out -- I want to insult their data scientists. Here is where ethical issues creep up (jacking up prices, monopolies, drivers making a living wage). Prices also change with respect to competitors, e.g. in oligopoly
- Stock markets and trading (bots), competition, supply/demand, etc
- Management consulting: e.g. if they want to enter a new market, build a new product, is also related to marketing and market research (liana, you gov), product deveylopment and many many use-cases which can be solved by such a consulting firm
- Medicine: new vaccine, treatment, drug development
- Banks and insurers: risk management, credit default (mortgage, etc). Chatbots and customer-support, for most frequent and simple questions.
- Automotive: e.g. automated parking, self-driving, safety warnings. Predictive maintainance is also a problem, replace risky pieces.
- The message is: at a closer look, if the service seems to do something relatively intelligent (e.g. reddit, netflix, amazon) -- content curation, recommendation, ranking, merchandising. the whole field of recommender systems

Again, everywhere, we do have to take decisions, we have uncertainty from multiple sources, complicated problems at a large scale (impossible to do manually without an army of employees).
...
%%| fig-width: 9
%%| fig-cap: |
%%|   In the next lecture we will get an insider's perspective of how it works
%%|   We'll also introduce a few new concepts, like full stack data apps, 
%%|   But now, we focus on trying to solve the right problem
...
philosophy: This is a course in which we look at the widest range possible of tools and models, wnithout going into depth, as I don't know which ones will be useful for your particular applications and problems. The idea is to find the right/optimal tool for the job when you encounter it, and then learn its details and how to actually do it. We go into more detail into workhorse-models, proven by practice to apply well in a large variety of cases. 

## Data Science in the Wild: applications
